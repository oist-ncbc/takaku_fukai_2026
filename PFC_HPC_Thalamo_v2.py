...
# -*- coding: utf-8 -*-
"""
Created on Tue Mar 14 11:53:31 2023
...
#Re_v2 revised training part to avoid implace error
...
def make_Ttraj_slow(direction):
    traj_x = np.array([])
    traj_y = np.array([])
    straight_x = np.ones(10)*0.5
    if direction == 1:
        branch1_x = np.linspace(0.5, 0.75, 10)
        branch2_x = np.linspace(0.75, 0.5, 10)
    elif direction == 2:
        branch1_x = np.linspace(0.5, 0.25, 10)
        branch2_x = np.linspace(0.25, 0.5, 10)        
    backstraight_x = np.ones(10)*0.5

    straight_y = np.linspace(0, 0.5, 10)
    branch1_y = np.linspace(0.5, 0.5, 10)
    branch2_y = np.linspace(0.5, 0.5, 10)
...
def make_traj_test(delay_length):
    ...
    for idx in order:
        ...
        data_x = np.append(data_x,target_x+np.random.normal(loc=0.5, scale=noise, size=target_x.shape),axis=0)
        data_y = np.append(data_y,target_y+np.random.normal(loc=0.5, scale=noise, size=target_y.shape),axis=0)
 
        
    print(data_x.shape,data_y.shape)
    return data_x[1:],data_y[1:]

def make_traj_slow(delay_length):
    freq=60
    noise=0.005
    data_length = 40
...
class MyLSTM_RNN(nn.Module):
    def __init__(self, input_size, hidden_size, output_size, batch_size, sparse):
        ...
        nn.init.sparse_(self.HPC.weight_hh.data,sparse)
        nn.init.sparse_(self.Re.weight_hh.data,sparse)
        # nn.init.normal_(self.PFC.weight_ih.data,0,1/2)
        # nn.init.normal_(self.HPC.weight_ih.data,0,1/2)
        # nn.init.normal_(self.Re.weight_ih.data,0,1/2)
        # nn.init.normal_(self.PFC.weight_hh.data,0,1/2)
        # nn.init.normal_(self.HPC.weight_hh.data,0,1/2)
        # nn.init.normal_(self.Re.weight_hh.data,0,1/2)
...
class MyLSTM_RNN_noise(nn.Module):
    ...
    def forward(self, input, hiddens):
        ...
        HPC_noised = hiddens[1][0] + torch.randn(self.batch_size, self.hidden_size_HPC)*c
        PFC_noised = hiddens[0][0] + torch.randn(self.batch_size, self.hidden_size_PFC)*c
        Re_noised = hiddens[2] + torch.poisson(torch.randn(self.batch_size, self.hidden_size_Re))*v
        Re_input = torch.cat([PFC_noised,HPC_noised],dim=1)
...
class MyLSTM_RNN_3input(nn.Module):
    def __init__(self, input_size, hidden_size, output_size, batch_size, sparse):
        super(MyLSTM_RNN_3input, self).__init__()
        ...
        self.PFC = nn.LSTMCell(self.hidden_size_HPC+self.hidden_size_Re, self.hidden_size_PFC)
        # self.PFC = nn.LSTMCell(hidden_size*2, hidden_size)
        self.HPC = nn.LSTMCell(3+self.hidden_size_Re, self.hidden_size_HPC)
        self.Re = nn.RNNCell(self.hidden_size_HPC+self.hidden_size_PFC, self.hidden_size_Re)
        self.linear = nn.Linear(self.hidden_size_HPC, output_size)
        ...
        # nn.init.normal_(self.HPC.weight_hh.data,0,1/10)
        # nn.init.normal_(self.Re.weight_hh.data,0,1/10)
        # nn.init.normal_(self.linear_input.data,0,3/10)

    def forward(self, input, hiddens):
        extended_input = self.linear_input(input)
        Re_input = torch.cat([hiddens[0][0],hiddens[1][0]],dim=1)
        Re_hidden = self.Re(Re_input, hiddens[2])
        HPC_input = torch.cat([extended_input,hiddens[2]],dim=1)
        HPC_hidden = self.HPC(HPC_input, hiddens[1])
        PFC_input = torch.cat([hiddens[1][0],hiddens[2]],dim=1)
        PFC_hidden = self.PFC(PFC_input, hiddens[0])
        output = self.linear(HPC_hidden[0])
...
def main():
    ...
    # model_path = 'model/ReModel_interRNNrand_AddRe_OUT5_long_s'+str(sparse)+'_200_2.pth'
    # model_path = 'model/ReModel_L2_interRNNrand_AddRe_OUT5_1212121_s'+str(sparse)+'_100_1_2.pth'
    # model_path = 'model/P+/ReModel_L2_interRNNrand_AddPFC100_OUT5_1'+str(delay_length)+'1_s'+str(sparse)+'_100_2_epoch100.pth'
    # model_path = 'model/R20_cont/ReModel_L2_interRNNrand_OUT1_1'+str(delay_length)+'1_s'+str(sparse)+'_100_2_epoch150.pth'
    # model_path = 'model/R20_cont/ReModel_L2_interRNNrand_OUT1_151_s8_100_3_epoch170.pth'
    # model_path = 'model/R20_cont/ReModel_L2_interRNNrand_OUT1_151_s1_100_4_epoch220.pth'
    # model_path = 'model/R20_cont/ReModel_L2_interRNNrand_OUT1_151_s6_100_4_epoch345.pth'
    # model_path = 'model/R20_131/ReModel_L2_interRNNrand_OUT1_131_s2_100_1_epoch195.pth'
    # rnn.load_state_dict(torch.load(model_path))
    
    model_path,past_epochnum = choose_maxmodel("model/R20_traintest/",sparse,index_num)
    rnn.load_state_dict(torch.load(model_path))
    ...
    for epoch in range(epochs_num):
        ...
        if epoch % 5 == 0:
            ...
            for k in range(data_limit):
                    ...
                    PFCstate.append(hidden_test[0][0].tolist())
                    HPCstate.append(hidden_test[1][0].tolist())
                    Restate.append(hidden_test[2][0].tolist())
            ...
            if correct_num > -1 or epoch == 50:
                # model_path = 'model/R20_transfer_3to5/ReModel_L2_interRNNrand_OUT1_transfers21_1'+str(delay_length)+'1_s'+str(sparse)+'_100_'+str(index_num)+'_epoch'+str(past_epochnum+epoch)+'.pth'
                # model_path = 'model/R20_1'+str(delay_length)+'1_rand_in/ReModel_L2_interRNNrand_OUT1_1'+str(delay_length)+'1_s'+str(sparse)+'_100_'+str(index_num)+'_epoch'+str(past_epochnum+epoch)+'.pth'
                model_path = 'model/R20_traintest/ReModel_L2_interRNNrand_OUT1_1'+str(delay_length)+'1_s'+str(sparse)+'_100_'+str(index_num)+'_epoch'+str(past_epochnum+epoch)+'.pth'
                # model_path = 'model/R20_transfer_test/ReModel_L2_interRNNrand_OUT1_transfers64_1'+str(delay_length)+'1_s'+str(sparse)+'_100_'+str(index_num)+'_epoch'+str(past_epochnum+epoch)+'.pth'
                ...
                s = str(past_epochnum+epoch) + "," + str(correct_num) + "," + model_path +"\n"
                with open("model/R20_traintest/correct_list.txt", mode="a") as f:
                    f.write(s)
    ...
    # model_path = 'model/ReModel_L2_interRNNrand_AddRe_OUT5_121'+str(delay_length)+'121_s'+str(sparse)+'_100_3.pth'
    # model_path = 'model/ReModel_L2_interRNNrand_AddRe_HPC5_OUT5_1'+str(delay_length)+'1_s'+str(sparse)+'_100_2.pth'
    # model_path = 'model/test2/ReModel_L2_interRNNrand_AddRe_PnoiseRe_OUT5_1'+str(delay_length)+'1_s'+str(sparse)+'_100_3.pth'
    # model_path = 'model/P+/ReModel_L2_interRNNrand_AddPFC100_OUT5_1'+str(delay_length)+'1_s'+str(sparse)+'_100_2add.pth'
    # model_path = 'model/R20_cont/ReModel_L2_interRNNrand_OUT1_1'+str(delay_length)+'1_s'+str(sparse)+'_100_10.pth'
    # model_path = 'model/R20_cont_1'+str(delay_length)+'1/ReModel_L2_interRNNrand_OUT1_1'+str(delay_length)+'1_s'+str(sparse)+'_100_'+str(index_num)+'.pth'
    # model_path = 'model/R20_feed/FeedModel_L2_interRNNrand_OUT1_1'+str(delay_length)+'1_s'+str(sparse)+'_100_2.pth'
    # model_path = 'model/ReModeltest_30reset3addlong_estref_L2_interRNNrand_Reinh_AddRe_OUT5_1'+str(delay_length)+'1_s'+str(sparse)+'_100_4.pth'
    ...
    # model_path = 'model/R20_uniHPC_1'+str(delay_length)+'1/ReModel_L2_interRNNrand_OUT1_121'+str(delay_length)+'121_s'+str(sparse)+'_100_'+str(index_num)+'.pth'
    # torch.save(rnn.state_dict(), model_path)
