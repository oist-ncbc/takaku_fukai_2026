...
# -*- coding: utf-8 -*-
"""
Created on Thu Dec  7 15:43:46 2023
...
# -*- coding: utf-8 -*-
"""
Created on Fri Apr 14 15:26:54 2023

@author: munenori
V2...Big batch model 20
"""

#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
Created on Mon Dec  6 15:06:31 2021
...
class MyLSTM_feedforward_Thalamus(nn.Module):
    def __init__(self, input_size, hidden_size, output_size, batch_size, sparse):
        super(MyLSTM_feedforward_Thalamus, self).__init__()
    ...
    def forward(self, input, hiddens):
        ...
        THinh_hidden = self.THinh(THinh_input)
        HPC_input = torch.cat([input,hiddens[2]],dim=1)
        HPC_hidden = self.HPC(HPC_input, hiddens[1])
        PFC_input = torch.cat([hiddens[1][0],hiddens[2]],dim=1)
        PFC_hidden = self.PFC(PFC_input, hiddens[0])
    ...
    def initHidden(self):
        ...
        PFC_hidden = [torch.zeros(self.batch_size, self.hidden_size), torch.zeros(self.batch_size, self.hidden_size)]
        Re_hidden = torch.zeros(self.batch_size, self.hidden_size)
        THinh_hidden = torch.rand(self.batch_size, self.hidden_size_THinh)
    ...
    def initHidden_rand(self):
        ...
        PFC_hidden = [torch.rand(self.batch_size, self.hidden_size_PFC)*var, torch.rand(self.batch_size, self.hidden_size_PFC)*var]
        Re_hidden = torch.rand(self.batch_size, self.hidden_size_Re)*const
        THinh_hidden = torch.rand(self.batch_size, self.hidden_size_THinh)*const
        return [PFC_hidden,HPC_hidden,Re_hidden,THinh_hidden]
...
def main():
    ...
    # rnn = MyLSTM_RNN_uniPFCHPC(inputsize, hidden_size, outputsize, batch_size, sparse)
    ...
    optimizer_Re = optim.Adam(rnn.Re.parameters(), lr=0.0002, weight_decay=1e-8)
    optimizer_THinh = optim.Adam(rnn.THinh.parameters(), lr=0.0002, weight_decay=1e-8)
    ...
    # model_path = 'model/ReModel_L2_interRNNrand_Reinh_AddRe_OUT5_H121_s'+str(sparse)+'_100_2.pth'
    ...
    model_path,past_epochnum = choose_maxmodel("model/R20_feedReinhReLU_H_bigbatch/",sparse,index_num)
    ...
    for epoch in range(epochs_num):
            ...
            while k < data.shape[0]-1:
                ...
                if np.any(ids==k):
                    ...
                    for n, p in rnn.named_parameters():
                        ...
                        #     p.data.clamp_(max=0) 
                        if n == "Re.weight":
                            p.data[:,hidden_size_HPC+hidden_size_PFC:].clamp_(max=0)
                        # if n == "THinh.weight":
                        #     p.data.clamp_(max=0)   
        ...
        if epoch % 5 == 0:
            ...
            if correct_num > -1 or epoch == 50:
                model_path = 'model/R20_feedReinhReLU_H_bigbatch/ReModel_L2_interRNNrand_OUT1_1'+str(delay_length)+'1_s'+str(sparse)+'_100_'+str(index_num)+'_epoch'+str(past_epochnum+epoch)+'.pth'
                # model_path = 'model/R20_H_stopinit_bigbatch/ReModel_L2_interRNNrand_OUT1_1'+str(delay_length)+'1_s'+str(sparse)+'_100_'+str(index_num)+'_epoch'+str(past_epochnum+epoch)+'.pth'
                # model_path = 'model/R20_uniHPC_H/ReModel_L2_interRNNrand_OUT1_1'+str(delay_length)+'1H_s'+str(sparse)+'_100_'+str(index_num)+'_epoch'+str(past_epochnum+epoch)+'.pth'
                ...
                with open("model/R20_feedReinhReLU_H_bigbatch/correct_list.txt", mode="a") as f:
                    f.write(s)
    ...
    # model_path = 'model/ReModel_L2_interRNNrand_AddRe_OUT5_H2v1'+str(delay_length)+'1_s'+str(sparse)+'_100_2.pth'
    # model_path = 'model/ReModel_L2_interRNNrand_AddRe_OUT5_H1'+str(delay_length)+'1_s'+str(sparse)+'_100_1.pth'
    # model_path = 'model/ReModeltest_30reset3addlong_estref_L2_interRNNrand_Reinh_AddRe_OUT5_1'+str(delay_length)+'1_s'+str(sparse)+'_100_1.pth'
